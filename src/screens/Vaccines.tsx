...
import { generateGoogleCalendarUrl, generateICSFile, downloadICS } from '@/src/utils/calendar';
import { Vaccine } from '@/src/types';

const Vaccines: React.FC = () => {
  const navigate = useNavigate();
  const { setIsVaccineFormOpen, isVaccineFormOpen } = useAppContext();
  ...
  React.useEffect(() => {
    setVaccines(OfflineService.getVaccines());
  }, [isVaccineFormOpen]);

  const historyVaccines = vaccines.filter(v => v.status === 'applied');

  const upcomingVaccines = vaccines.filter(v => v.status === 'upcoming' || v.status === 'overdue').map(v => {
    const daysLeft = v.date ? Math.ceil((new Date(v.date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)) : 0;
    return { ...v, daysLeft };
  });

  const displayVaccines = activeTab === 'history' ? historyVaccines : upcomingVaccines;

  const handleSyncCalendar = (e: React.MouseEvent, vaccine: Vaccine) => {
    e.stopPropagation();
    const url = generateGoogleCalendarUrl({
      title: `Vacina: ${vaccine.name}`,
      description: `Lembrete de vacinação para o pet via PetHub.`,
      startTime: new Date(vaccine.date),
      location: vaccine.clinic || 'Clínica Veterinária'
    });
    window.open(url, '_blank');
  };

  const handleDownloadICS = (e: React.MouseEvent, vaccine: Vaccine) => {
    e.stopPropagation();
    ...
    downloadICS(vaccine.name, content);
  };
            ...
            >
              ...
              <div className="flex-1 pb-6">
                <div className={`bg-surface rounded-3xl p-5 shadow-soft border border-border/50 hover:border-primary/30 hover:shadow-soft-lg transition-all duration-300 cursor-pointer active:scale-[0.99] relative overflow-hidden`}>
                  ...
                  <div className="flex justify-between items-start mb-2">
                    ...
                    {vaccine.status === 'upcoming' && (
                      <div className="flex gap-2">
                        <button
                          onClick={(e) => handleSyncCalendar(e, vaccine)}
                          className="bg-primary/10 hover:bg-primary/20 p-1.5 rounded-lg text-primary transition-colors"
                          title="Google Calendar"
                        >
                          <span className="material-symbols-outlined text-sm">calendar_add_on</span>
                        </button>
                        <button
                          onClick={(e) => handleDownloadICS(e, vaccine)}
                          className="bg-primary/10 hover:bg-primary/20 p-1.5 rounded-lg text-primary transition-colors"
                          title="Baixar .ics (iOS/Outlook)"
                        >
                          <span className="material-symbols-outlined text-sm">ios_share</span>
                        </button>
                        <span className="bg-primary/10 text-primary text-[10px] font-bold px-2 py-1 rounded-lg uppercase tracking-wider flex items-center">
                          Próxima
                        </span>
                      </div>
                    )}
                    {vaccine.status === 'overdue' && (
                      <span className="bg-red-100 text-red-600 text-[10px] font-bold px-2 py-1 rounded-lg uppercase tracking-wider">
                        Atrasada
                      </span>
                    )}
                  </div>
                  ...
                  {/* Use a type assertion or ensure the mapped type has daysLeft if extending interface */}
                  {(vaccine as any).daysLeft !== undefined && (vaccine as any).daysLeft > 0 && (
                    <div className="mt-3 flex items-center gap-2">
                      ...
                      <span className="text-primary text-xs font-bold whitespace-nowrap">
                        Faltam {(vaccine as any).daysLeft} dias
                      </span>
                    </div>
                  )}
                </div>
